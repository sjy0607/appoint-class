...
Page({
  ...
  toCardDetail(e){
    ...
    wx.navigateTo({
     url: '/pages/cardetail/index?type='+type+'&id='+id,
    })
  },
  getCardsList(customerId,code,_this){
    var encrypt = util.HMAC_sort([customerId], code);
    util.Req(app.globalData.url+app.globalData.config + '/member-card/usable/list', 'get', {customerId: customerId,digest:encrypt}, {
  ...
  onLoad: function (options) {
    ...
    if(options.hasOwnProperty('isDisable')) {
      this.setData({
        isDisabled:true
      })
    }
